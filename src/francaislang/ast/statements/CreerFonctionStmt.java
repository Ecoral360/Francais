...
import francaislang.ast.expressions.VarExpr;
import francaislang.execution.FrancaisLangExecutorState;
import francaislang.objects.FrancaisLangObj;
import francaislang.objects.function.FrancaisLangFonction;
 ...
 */
public class CreerFonctionStmt extends FrancaisLangStatement {
     ...
     * est appelée
     */
    public CreerFonctionStmt(String nom, List<VarExpr> args, ASCExecutor<FrancaisLangExecutorState> executeurInstance) {
        ...
        scope.declareVariable(new ASCVariable<>(nomFonction, FrancaisLangObj.AUCUNE_VALEUR));
    ...
    public Object execute() {
        ...
        var fonction = new FrancaisLangFonction(this.nomFonction,
                args.stream().map(arg -> new ASCVariable<>(arg.nom(), FrancaisLangObj.AUCUNE_VALEUR)).toList(),
                callingCoord,
                (ASCExecutor<FrancaisLangExecutorState>) executorInstance);

        variable.setAscObject(fonction);

        for (var arg : this.args) {
            scope.declareVariable(new ASCVariable<>(arg.nom(), FrancaisLangObj.AUCUNE_VALEUR));
        }
