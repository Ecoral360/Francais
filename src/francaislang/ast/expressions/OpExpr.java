...
public record OpExpr(Expression<?> left, Expression<?> right, String op) implements Expression<ASCObject<?>> {
    ...
    public ASCObject<?> eval() {
        ...
        if (op.equals("concaténé à")) {
        ...
        } else if (leftValue instanceof FrancaisLangNombre codeMdrNumberLeft && rightValue instanceof FrancaisLangNombre codeMdrNumberRight) {
            boolean unsigned = leftValue instanceof FrancaisLangEntierNonSigne && rightValue instanceof FrancaisLangEntierNonSigne;
            ...
            return result != (int) result ? new FrancaisLangDecimal(result)
                    : unsigned ? new FrancaisLangEntierNonSigne((int) result)
                    : new FrancaisLangEntier((int) result);
        } else {
