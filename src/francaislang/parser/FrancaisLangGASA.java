 ...
 * </ul>
 */
public class FrancaisLangGASA extends AstGenerator<FrancaisLangAstFrameKind> {
    private ASCExecutor<FrancaisLangExecutorState> executorInstance;
     ...
     */
    public FrancaisLangGASA(ASCExecutor<FrancaisLangExecutorState> executorInstance) {
        setPatternProccessor(executorInstance.<FrancaisLangJetoniseur>getLexer()::remplaceCategoriesByMembers);
        defineAstFrame(FrancaisLangAstFrameKind.DEFAULT);
        addStatements();
        addExpressions();
        pushAstFrame(FrancaisLangAstFrameKind.DEFAULT);
        this.executorInstance = executorInstance;
    }

    public ASCExecutor<FrancaisLangExecutorState> getExecutorInstance() {
     ...
     */
    public void setExecutorInstance(ASCExecutor<FrancaisLangExecutorState> executorInstance) {
    ...
    protected void addStatements() {
        ...
        addStatement("APPELER expression AVEC ARG expression~" +
                        ...
                        "APPELER expression",
                (p, variant) ->
                        FrancaisLangStatement.evalExpression(executorInstance, switch (variant) {
        ...
        addStatement("expression", p -> FrancaisLangStatement.evalExpression(executorInstance, (Expression<?>) p.get(0)));
        addStatement("", p -> FrancaisLangStatement.statementVide(executorInstance));
    }
    ...
    protected void addExpressions() {
        ...
        addExpression("{datatypes}~VARIABLE~{datatypes_name}", p -> {
            ...
            return switch (token.name()) {
                case "ENTIER" -> new ConstValueExpr(new FrancaisLangEntier(token));
                case "HEX" -> new ConstValueExpr(new FrancaisLangEntierNonSigne(Integer.parseUnsignedInt(token.value().substring(2), 16)));
                case "DECIMAL" -> new ConstValueExpr(new FrancaisLangDecimal(token));
                case "TEXTE" -> new ConstValueExpr(new FrancaisLangTexte(token));
                case "VARIABLE" -> new VarExpr(token.value(), executorInstance.getExecutorState());
                default -> new ConstValueExpr(new FrancaisLangType(token));
            };
        });
